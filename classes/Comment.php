<?php class Comment {
	...
	public function __construct(int $comment_id, int $post_event_id, bool $isPost = true) {
		if ($isPost) {
			$connection = Database::getConnection();
			$sql = "SELECT * FROM comments WHERE id=$comment_id AND post_id=$post_event_id";
			...
			$connection = Database::getConnection();
			$sql = "SELECT * FROM comments WHERE id=$comment_id AND event_id=$post_event_id";
			$result = mysqli_query($connection, $sql);
			$connection->close();
	...
	public function render() {
		...
		$comment_style = "style='padding: 20px 20px 40px 20px; margin: 20px 20px 20px 20px;border-radius: 20px;'";
			...
			<div class='row'>
				<div class='white z-depth-1' $comment_style>
					<div class='black-text' style='word-wrap: break-word;'>
						...
						<span>
							<p style='margin: 10px 0 10px 0'>$comment_body</p>
						</span>
					</div>
